			...
			for(var/i in 1 to amount)
				...
				if(isitem(new_item) && !istype(new_item, /obj/item/stack/sheet)) // To avoid materials dupe glitches
					...
					if(locked)
						...
						new_item_item.forceMove(lockbox)
						lockbox.name += " ([new_item_item.name])"
						lockbox.origin_tech = new_item_item.origin_tech
						lockbox.req_access = being_built.access_requirement
						lockbox.w_class = new_item_item.w_class > lockbox.w_class ? new_item_item.w_class : lockbox.w_class
