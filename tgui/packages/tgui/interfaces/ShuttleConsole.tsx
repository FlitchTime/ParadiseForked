import { useBackend } from '../backend';
import {
  Box,
  Button,
  Dropdown,
  Flex,
  Icon,
  LabeledList,
  Modal,
  Section,
} from '../components';
import { Window } from '../layouts';

type ShuttleConsoleData = {
  status: string;
  locked: boolean;
  authorization_required: boolean;
  destination: string;
  docked_location: string;
  timer_str: string;
  locations: Location[];
};

type Location = {
  name: string;
  id: string;
};

type ShuttleConsoleProps = {
  type?: string;
  blind_drop: boolean;
};

export const ShuttleConsole = (props: ShuttleConsoleProps) => {
  const { act, data } = useBackend<ShuttleConsoleData>();
  const { type = 'shuttle', blind_drop } = props;
  const { authorization_required } = data;
  return (
    <Window width={350} height={240}>
      {!!authorization_required && (
        <Modal
          ml={1}
          mt={1}
          width={26}
          height={12}
          fontSize="28px"
          fontFamily="monospace"
          textAlign="center"
        >
          <Flex>
            <Flex.Item mt={2}>
              <Icon name="minus-circle" />
            </Flex.Item>
            <Flex.Item mt={2} ml={2} color="bad">
              {type === 'shuttle' ? 'SHUTTLE LOCKED' : 'BASE LOCKED'}
            </Flex.Item>
          </Flex>
          <Box fontSize="18px" mt={4}>
            <Button
              lineHeight="40px"
              icon="arrow-circle-right"
              content="Request Authorization"
              color="bad"
              onClick={() => act('request')}
            />
          </Box>
        </Modal>
      )}
      <Window.Content>
        <ShuttleConsoleContent type={type} blind_drop={blind_drop} />
      </Window.Content>
    </Window>
  );
};

const getLocationNameById = (locations: Location[], id: string) => {
  return locations?.find((location: Location) => location.id === id)?.name;
};

const getLocationIdByName = (locations: Location[], name: string) => {
  return locations?.find((location: Location) => location.name === name)?.id;
};

const STATUS_COLOR_KEYS = {
  'В пути': 'good',
  'Ожидание': 'average',
  'Запуск': 'average',
  'Зарядка': 'average',
  'Потерянный': 'bad',
  'Несанкционированный доступ': 'bad',
  'Заблокирован': 'bad',
};

export const ShuttleConsoleContent = (props: ShuttleConsoleProps) => {
  const { act, data } = useBackend<ShuttleConsoleData>();
  const { type, blind_drop } = props;
  const {
    status,
    locked,
    authorization_required,
    destination,
    docked_location,
    timer_str,
    locations = [],
  } = data;
  return (
    <Section>
      <Box bold fontSize="26px" textAlign="center" fontFamily="monospace">
        {timer_str || '00:00'}
      </Box>
      <Box textAlign="center" fontSize="14px" mb={1}>
        <Box inline bold>
          Статус:
        </Box>
        <Box inline color={STATUS_COLOR_KEYS[status] || 'bad'} ml={1}>
          {status || 'Not Available'}
        </Box>
      </Box>
      <Section
        title={type === 'shuttle' ? 'Управление шаттлом' : 'Base Launch Controls'}
      >
        <LabeledList>
          <LabeledList.Item
          label="Местоположение"
          >
            {docked_location || 'Недоступно'}
          </LabeledList.Item>
          <LabeledList.Item
            label="Место назначения"
            buttons={
              type !== 'shuttle' &&
              locations.length === 0 &&
              !!blind_drop && (
                <Button
                  color="bad"
                  icon="exclamation-triangle"
                  disabled={authorization_required || !blind_drop}
                  onClick={() => act('random')}
                >
                  Blind Drop
                </Button>
              )
            }
          >
            {(locations.length === 0 && (
              <Box
              mb={1.7}
              color="bad"
              >
                Недоступно
              </Box>
            )) ||
              (locations.length === 1 && (
                <Box
                mb={1.7}
                color="average"
                >
                  {getLocationNameById(locations, destination)}
                </Box>
              )) || (
                <Dropdown
                  over
                  options={locations.map((location) => location.name)}
                  disabled={locked || authorization_required}
                  selected={
                    getLocationNameById(locations, destination) ||
                    'Выбрать место назначения'
                  }
                  onSelected={(value) =>
                    act('set_destination', {
                      destination: getLocationIdByName(locations, value),
                    })
                  }
                />
              )}
          </LabeledList.Item>
          <LabeledList.Item />
        </LabeledList>
        <Button
          fluid
          disabled={
            !getLocationNameById(locations, destination) ||
            locked ||
            authorization_required ||
            status !== 'Ожидание'
          }
          icon="arrow-up"
          textAlign="center"
          onClick={() =>
            act('move', {
              shuttle_id: destination,
            })
          }
        >
          Отправить
        </Button>
      </Section>
    </Section>
  );
};
